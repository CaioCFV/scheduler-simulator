...
function Media({ data }) {
  ...
  data.process.map(
    (item) => (medias.arround += item.wait - item.arrived + item.execution)
  );
  ...
  if (data.process.length) {
    return (
      <tr>
        ...
        <td>
          {medias.arround <= 0 ? 0 : medias.arround / data.process.length}
        </td>
      </tr>
    );
  } else {
    ...
    <>
      ...
      <Square>
        {data.steps.map((item, i) => {
          return (
            <Item key={i} background={item.color}>
              <Time className="a">
                <i>{i + 1}</i>
              </Time>
              <Info>
            ...
            {data.process.map((item, i) => {
              return (
                <tr key={i}>
                  ...
                  <td>{item.name}</td>
                  <td>
                    {item.wait - item.arrived < 0
                      ? 0
                      : item.wait - item.arrived}
                  </td>
                  <td>
                    {item.status === "false"
                      ? 0
                      : item.wait - item.arrived + item.execution}
                  </td>
                </tr>
              );
            })}
